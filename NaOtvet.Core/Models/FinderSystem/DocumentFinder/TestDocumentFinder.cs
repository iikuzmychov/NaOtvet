...
{
    public class TestDocumentFinder
    {
        ...
        private void FindTestDocument()
        {
            ...
            for (int i = StartIndex; i < StartIndex + IterationsCount && !IsStopped; i++)
            {
                for (int j = 0; j < 2 && !IsStopped; j++)
                {
                    ...
                    try
                    {
                        isCorrectTestDocument = IsCorrectTestDocument(client, TestSession, testDocumentId, out flashCards);
                    }
        ...
        public static bool IsCorrectTestDocument(NaUrokClient client, TestSession testSession, int testDocumentId, out FlashCard[] flashCards)
        {
            flashCards = client.GetFlashCards(testDocumentId);
